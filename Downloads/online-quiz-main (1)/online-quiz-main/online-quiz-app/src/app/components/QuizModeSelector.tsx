...
export function QuizModeSelector({ onSelect, onCancel, category }: QuizModeSelectorProps) {
  ...
  const [selectedMode, setSelectedModeInternal] = useState<QuizMode | null>(null);
      ...
      >
        <Card className="bg-white/95 backdrop-blur-md shadow-2xl border-2 border-white/50 overflow-hidden">
          ...
          <CardContent className="p-8">
            ...
            <AnimatePresence>
              {selectedMode === 'ai' && !showMultiplayerMenu && (
                <motion.div 
                  initial={{ opacity: 0, height: 0 }}
                  animate={{ opacity: 1, height: 'auto' }}
                  exit={{ opacity: 0, height: 0 }}
                  className="max-w-md mx-auto p-6 bg-emerald-50 rounded-2xl border-2 border-emerald-200 mb-10 overflow-hidden"
                >
                  <h4 className="font-bold text-emerald-800 mb-3 flex items-center justify-center gap-2 text-lg">
                    <Zap className="w-6 h-6 text-emerald-500 fill-emerald-500"/> What should the quiz be about?
                  </h4>
                  <div className="flex gap-3">
                    <input 
                      ...
                      id="ai-topic-input"
                      autoFocus
                      placeholder="e.g. Quantum Physics, Taylor Swift, World War II..."
                      className="flex-1 px-4 py-3 border-2 border-emerald-100 rounded-xl focus:border-emerald-500 focus:outline-none shadow-sm text-lg"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          onSelect('ai', (e.target as HTMLInputElement).value);
                        }
                      }}
                    />
                    <Button 
                      onClick={() => {
                        ...
                        if(input.value) onSelect('ai', input.value);
                      }}
                      className="bg-emerald-500 hover:bg-emerald-600 h-auto px-6 rounded-xl font-bold shadow-lg shadow-emerald-200"
                    >
                      Go
                    </Button>
                  </div>
                  <p className="text-emerald-600 text-xs mt-3 text-center opacity-80 italic">Gemini will build a custom quiz just for you!</p>
                </motion.div>
              )}
            </AnimatePresence>
