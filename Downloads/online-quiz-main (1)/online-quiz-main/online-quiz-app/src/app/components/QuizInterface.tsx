...
import { AlertCircle, Timer, Zap, Trophy, TrendingUp, Skull, Shield, CheckCircle2, Bot, ArrowRight, ArrowLeft, CheckCircle, XCircle } from 'lucide-react';
import { api, createMultiplayerSocket } from '@/services/api';
...
export function QuizInterface({ questions, category, difficulty, mode, roomId, username, onComplete }: QuizInterfaceProps) {
  ...
  useEffect(() => {
    if (mode === 'multiplayer' && roomId && username) {
      const socket = createMultiplayerSocket(roomId);
