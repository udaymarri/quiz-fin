...
export const api = {
  ...
  generateAiQuiz: async (topic: string, difficulty: string): Promise<Question[]> => {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 20000); // 20s for AI gen
  ...
  sendChatbotMessage: async (message: string): Promise<{response: string}> => {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 15000); // 15s for Chatbot
...
export const createMultiplayerSocket = (roomId: string) => {
  return new WebSocket(`${WS_BASE_URL}/ws/multiplayer/${roomId}`);
};
